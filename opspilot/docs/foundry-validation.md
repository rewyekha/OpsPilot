...
**GPT-4o (commander)**, **GPT-4o-mini (specialist)**, and **o3 (reasoning)**.
...
| `REASONING_MODEL_DEPLOYMENT` | deployment for `REASONING` role | `o3` |
...
## 2. Reasoning-model (o3) parameter handling

`FoundryProvider` is hardened so the same code path is valid for chat **and** reasoning models:

- **No `temperature`** is sent for the `REASONING` role (o3 rejects non-default temperature). Chat models get `temperature=0.2`.
- **No `max_tokens`** is sent (o3 uses `max_completion_tokens`); service defaults apply — no unsupported token parameter is ever passed.
- **Structured output** uses `beta.chat.completions.parse(response_format=<PydanticModel>)` with **no temperature**, keeping it valid for gpt-4o / gpt-4o-mini / o3.
...
|---|---|---|---|
| mock | `mock` | commander→`gpt-4o`, specialist→`gpt-4o-mini`, reasoning→`o3` | deterministic `[MOCK · <role> · <model>] …` |
| foundry | `foundry` | same deployment names | live model text |

```json
{ "provider": "mock", "model": "o3", "role": "reasoning", "response": "[MOCK · reasoning · o3] …" }
```
...
| o3 rejects `temperature`/`max_tokens` | wrong params | already handled — `REASONING` sends neither; verify a custom deployment name maps to an actual o3 deployment |
